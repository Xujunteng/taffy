      ...
      <div class="card">
        ...
        <div v-if="trend.length" class="svg-chart">
          <svg :viewBox="`0 0 ${trend.length * 80 + 40} 180`" width="100%" height="180" style="display:block">
            <!-- 网格线 -->
            <line v-for="y in 4" :key="'g'+y" :x1="40" :y1="y*35" :x2="trend.length*80+20" :y2="y*35" stroke="var(--border)" stroke-dasharray="4,4" />
            <!-- 柱状条 -->
            <g v-for="(item, i) in trend" :key="i">
              <rect :x="i*80+55" :y="160 - item.height" width="44" :height="item.height" rx="4" :fill="item.color || 'var(--chart-1)'" />
              <text :x="i*80+77" :y="158 - item.height" text-anchor="middle" font-size="12" fill="var(--text)">{{ item.count }}</text>
              <text :x="i*80+77" y="178" text-anchor="middle" font-size="11" fill="var(--muted)">{{ item.label }}</text>
            </g>
          </svg>
        </div>
...
.activity-item { display:flex; align-items:center; padding:8px 0; border-bottom:1px solid var(--border); }
