  ...
  <div class="page page-narrow">
    ...
    <div class="grid grid-4" v-loading="loading">
      <div class="card stat-card" @click="$router.push('/voices')" style="cursor:pointer">
        <div class="stat-icon stat-tone-1"><el-icon :size="28"><Microphone /></el-icon></div>
      ...
      <div class="card stat-card" @click="$router.push('/tts')" style="cursor:pointer">
        <div class="stat-icon stat-tone-2"><el-icon :size="28"><Headset /></el-icon></div>
      ...
      <div class="card stat-card" @click="$router.push('/scripts')" style="cursor:pointer">
        <div class="stat-icon stat-tone-3"><el-icon :size="28"><Document /></el-icon></div>
      ...
      <div class="card stat-card" @click="$router.push('/stats')" style="cursor:pointer">
        <div class="stat-icon stat-tone-4"><el-icon :size="28"><VideoPlay /></el-icon></div>
      ...
      <div class="card" style="grid-column:span 2">
        ...
        <div v-if="recent.length" class="timeline">
          <div v-for="(item, i) in recent" :key="i" class="timeline-item">
            <div class="tl-dot" :class="item.type === 'tts' ? 'is-tts' : 'is-feedback'"></div>
...
.tl-content p { margin:4px 0 2px; color:var(--text); }
.stat-tone-1 { background:color-mix(in srgb,var(--chart-1) 13%,transparent); color:var(--chart-1); }
.stat-tone-2 { background:color-mix(in srgb,var(--chart-2) 13%,transparent); color:var(--chart-2); }
.stat-tone-3 { background:color-mix(in srgb,var(--chart-3) 13%,transparent); color:var(--chart-3); }
.stat-tone-4 { background:color-mix(in srgb,var(--chart-4) 13%,transparent); color:var(--chart-4); }
.tl-dot.is-tts { background:var(--chart-1); }
.tl-dot.is-feedback { background:var(--chart-2); }
</style>
